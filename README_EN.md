...
<p align="center">
  ...
  <a href="#native-build">Native Build</a> ·
...
### Platform Support
- **H5 Web** - Access directly in browser
- **Android** - Packaged as native APK/AAB via Capacitor
- **iOS** - Packaged as native IPA via Capacitor (requires macOS)
...
# Sync web assets to native projects only
npm run cap:sync
```
...
| **Capacitor** | Cross-platform native packaging (iOS / Android) |
...
```
calorie-calculation/
├── capacitor.config.ts   # Capacitor config (splash screen, status bar, etc.)
...
│   │   ├── storage.ts
│   │   ├── ai.ts
│   │   └── capacitor.ts  # Capacitor native lifecycle
│   ├── utils/            # Utilities
